...
def is_blocked_brand(name: str) -> bool:
    """True if the venue name matches a known chain (curation spec 2.4)."""
    n = normalize_name(name or "")
    if n in CHAIN_EXACT:
        return True
    return any(n == p or n.startswith(p + " ") for p in CHAIN_PREFIX)
