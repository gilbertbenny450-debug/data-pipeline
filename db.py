...
def eligible(v) -> bool:
    """Universe rules: open, independent/small-group, not a known chain (spec 2.4)."""
    if not v["is_open"]:
        return False
    if is_blocked_brand(v["name"]):
        return False
    return (v["chain_sites"] or 1) <= CHAIN_MAX_SITES
...
def compute(db_path="london_food.db", top=15):
    ...
    for v in venues:
        ...
        # Upcoming (spec 3.2) — requires corroboration: editorial buzz or a
        # Companies House match. First appearance in our data alone is NOT
        # evidence a venue is new (OSM mappers add old venues all the time).
        ...
        recently_seen = age_days <= UPCOMING_GRADUATE_DAYS and v["first_seen"] != bootstrap_date
        if is_new_biz or (recently_seen and buzz > 0):
