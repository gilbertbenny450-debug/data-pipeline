...
CREATE TABLE IF NOT EXISTS venues (
  ...
  rating REAL,            -- filled later when Places is enabled
  review_count INTEGER,   -- filled later when Places is enabled
...
def upsert_venue(con, v: dict):
    ...
    con.execute("""
      INSERT INTO venues (osm_id,name,norm_name,category,cuisine,lat,lon,region,postcode,last_seen)
      VALUES (:osm_id,:name,:norm_name,:category,:cuisine,:lat,:lon,:region,:postcode,date('now'))
...
def mark_closed(con, days_unseen=21):
    """Venues absent from recent pulls are flagged closed (data freshness, spec)."""
    con.execute("""
